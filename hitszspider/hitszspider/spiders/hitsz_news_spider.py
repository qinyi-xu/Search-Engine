...
class HITSZNewsSpider(scrapy.Spider):
    # 用于区别Spider
    name = "HITSZNewsSpider"
    # 允许访问的域
    allowed_domains = ['hitsz.edu.cn']  # 修改
    # 爬取的起始地址
    start_urls = ['http://www.hitsz.edu.cn/article/index.html']
    # 将要爬取的地址列表
    ...
    def parse(self, response):
        ...
        if response.url.startswith("http://www.hitsz.edu.cn/article/view/"):
            item = NewsItem()
            for box in response.xpath('//div[@class="container_news"]/div[@class="mainwidth"]/div[@class="leftside_news"]/div[@class="detail_out"]'):   # 修改
                # article title
        ...
        for url in urls:
            ...
            if not real_url.startswith('http://www.hitsz.edu.cn'):
